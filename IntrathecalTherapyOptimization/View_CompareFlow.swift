...
class View_CompareFlow: UIViewController, UITextFieldDelegate
{
    ...
    func genMultiRateGraph() -> Void
    {
        ...
        let maxYValue = multirateDoseSlider1.maximumValue
        yCoord = (multirateDoseSlider1.value / maxYValue) * graphHeight
        ...
        var prevSliderVal = multirateDoseSlider2.value
        while(c < stepperState)
        {
            ...
            if(stepperState - c == 1)
            {
                ...
                if(stepperState == 2){ prevSliderVal = multirateDoseSlider2.value }
                if(stepperState == 3){ prevSliderVal = multirateDoseSlider3.value }
                if(stepperState == 4){ prevSliderVal = multirateDoseSlider4.value }
                date2 = multirateStartField1.text
            }
            else if(c == 0){ sliderVal = multirateDoseSlider2.value }
            else if(c == 1){ sliderVal = multirateDoseSlider3.value }
            else if(c == 2){ sliderVal = multirateDoseSlider4.value }
